...
class HighScoreRepository:
    ...
    def check_if_top_ten(self, score, level):
        ...
        lowest_high_score = self.cursor.execute(lowest_high_score, {'level': level}).fetchone()

        if lowest_high_score['score'] is None:
            return True
    ...
    def delete_excess_rows(self):
        ...
        self.cursor.execute(f'DELETE FROM high_scores WHERE id NOT IN ({high_scores})')
    ...
    def get_top_ten(self, level):
        ...
        high_scores = self.cursor.execute(high_scores, {'level': level}).fetchall()
